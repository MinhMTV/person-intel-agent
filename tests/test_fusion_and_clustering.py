...
def face(band: FaceMatchBand, distance: float) -> FaceEvidence:
    return FaceEvidence(
        ...
        reference_face_id="f1",
        candidate_image_url=URL + "/i.jpg",
...
def text(type_: EvidenceType, strength=EvidenceStrength.MODERATE, field="name", exact=True):
    return make_evidence(
        type_, strength, "obs", URL, text=TextMatch(field=field, expected="x", observed="x", exact=exact)
    )


def exact_image():
    return image_occurrence_evidence(
        ImageDiscoveryResult(provider="p", match_type=ImageMatchType.EXACT, image_url=URL + "/r.jpg", page_url=URL), URL
    )
...
def test_contradicting_face_caps_level():
    ...
    ctx = [
        text(EvidenceType.NAME_MATCH),
        text(EvidenceType.LOCATION_MATCH, field="location"),
...
def test_same_common_name_does_not_merge(tmp_path):
    out = clustering(tmp_path).cluster(
        [_node("https://a.example.com/1", name="Jane Doe"), _node("https://b.example.com/2", name="Jane Doe")]
    )
    assert len(out) == 2
...
def test_name_plus_handle_merges(tmp_path):
    out = clustering(tmp_path).cluster(
        [
            _node("https://github.com/janedoe93", name="Jane Doe", usernames=["janedoe93"]),
...
def test_explicit_cross_link_merges_and_creates_evidence(tmp_path):
    out = clustering(tmp_path).cluster(
        [_node("https://a.example.com/me", links=["https://b.example.net/jane"]), _node("https://b.example.net/jane")]
    )
    assert len(out) == 1
...
def test_face_similarity_alone_does_not_merge(tmp_path):
    ...
    out = clustering(tmp_path).cluster(
        [_node("https://a.example.com/1", emb=e), _node("https://b.example.com/2", emb=e)]
    )
    assert len(out) == 2
...
def test_irrelevant_pages_are_not_candidates(tmp_path):
    ...
    out = clustering(tmp_path).cluster(
        [_node("https://a.example.com/1", evidence=[face_evidence(f, "https://a.example.com/1")])]
    )
    assert out == []
