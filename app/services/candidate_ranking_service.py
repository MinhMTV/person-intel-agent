...
class CandidateRankingService:
    ...
    def rank(self, candidates: list[CandidateIdentity], hints: IdentityHints, face_available: bool) -> list[CandidateIdentity]:
        for candidate in candidates:
            candidate.assessment = self.fusion.assess(candidate, hints, face_available)
            candidate.evidence.sort(key=lambda e: (e.strength.value != "STRONG", e.strength.value != "MODERATE", e.type.value))
    ...
    def conclusion(candidates: list[CandidateIdentity]) -> str:
        ...
        if strong:
            ...
            return (f"{len(strong)} candidate(s) with strong evidence. Top: {top.display_name} "
                    f"({top.assessment.level.value}){extra}. Candidate matches must be verified independently.")
        if moderate:
            return (f"No strong candidate found. {len(moderate)} candidate(s) with moderate evidence need "
