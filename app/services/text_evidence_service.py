...
def page_corpus(page: CandidatePage) -> str:
    p = page.profile
    parts = [page.title, page.snippet, p.title, p.description, p.profile_name, p.site_name, p.text_excerpt,
             " ".join(p.locations), " ".join(p.organizations)]
...
class TextEvidenceService:
    def evaluate(self, page: CandidatePage, hints: IdentityHints) -> list[Evidence]:
        ...
        def add(type_: EvidenceType, strength: EvidenceStrength, field: str, expected: str, observed: str,
                text: str, exact: bool = True) -> None:
            out.append(make_evidence(type_, strength, text, url,
        ...
        if hints.name:
            ...
            if len(tokens) >= 2 and (profile_name == full or _phrase_in(name_corpus, full)):
                ...
                add(EvidenceType.NAME_MATCH, EvidenceStrength.MODERATE, "name", hints.name, observed or full,
                ...
                add(EvidenceType.NAME_MATCH, EvidenceStrength.WEAK, "name", hints.name, f"{tokens[0]} … {tokens[-1]}",
                    f"The page mentions “{tokens[0]}” and “{tokens[-1]}” but not the full name together.", exact=False)
        ...
        for username in hints.usernames:
            ...
            if u in page_usernames:
                add(EvidenceType.USERNAME_MATCH, EvidenceStrength.STRONG, "username", username, u,
                    f"The account handle on this profile is “{u}”, identical to a supplied username.")
            elif len(u) >= 5 and _phrase_in(corpus, u):
                add(EvidenceType.USERNAME_MATCH, EvidenceStrength.MODERATE, "username", username, _excerpt(corpus, u),
                    f"The page mentions the supplied username “{u}”.", exact=False)
        ...
        for email in hints.emails:
            ...
            if e and (e in page_emails or email.lower() in corpus):
                add(EvidenceType.EMAIL_MATCH, EvidenceStrength.STRONG, "email", email, email,
        ...
        if canonical_url(url) in known or (page.profile.canonical_url and page.profile.canonical_url in known):
            add(EvidenceType.KNOWN_URL, EvidenceStrength.STRONG, "known_url", url, url,
        ...
        if hints.location:
            ...
            if _phrase_in(corpus, city):
                add(EvidenceType.LOCATION_MATCH, EvidenceStrength.MODERATE, "location", hints.location,
                    _excerpt(corpus, city), f"The source mentions the location “{hints.location}”.")
            else:
                for term in expansion.search_terms[1:]:
                    t = normalize_text(term)
                    if len(t) >= 3 and _phrase_in(corpus, t):
                        add(EvidenceType.LOCATION_MATCH, EvidenceStrength.WEAK, "location", hints.location,
                            _excerpt(corpus, t), f"The source mentions “{term}”, a broader area containing "
                            f"“{hints.location}”.", exact=False)
                        break
        if hints.country and not any(e.type == EvidenceType.LOCATION_MATCH for e in out):
            ...
            if len(c) >= 3 and _phrase_in(corpus, c):
                add(EvidenceType.LOCATION_MATCH, EvidenceStrength.WEAK, "country", hints.country, _excerpt(corpus, c),
                    f"The source mentions the country “{hints.country}”.", exact=False)
