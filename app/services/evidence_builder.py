...
def make_evidence(
    type_: EvidenceType, strength: EvidenceStrength, observation: str, source_url: str | None, **fields: Any
) -> Evidence:
    ev = Evidence(
        id="",
        type=type_,
...
def image_occurrence_evidence(hit: ImageDiscoveryResult, source_url: str) -> Evidence:
    if hit.match_type == ImageMatchType.EXACT:
        ...
        what = (
            "a cropped/modified copy of the reference photo"
            if hit.match_type == ImageMatchType.PARTIAL
            else "an edited copy of the reference photo"
        )
    else:
    ...
    return make_evidence(
        type_,
        strength,
        f"{source_url} contains {what} — reported by {via}.",
        source_url,
...
def face_evidence(face: FaceEvidence, source_url: str) -> Evidence:
    if face.match_band in (FaceMatchBand.LOW, FaceMatchBand.NO_MATCH):
        text = (
            f"A face on an image from this source was compared with the target face and does NOT look like "
            f"the same person (band {face.match_band.value}, {face.model} cosine similarity {face.cosine_similarity:.3f})."
        )
    else:
        text = (
            f"A face on an image from this source has {face.match_band.value.replace('_', ' ')} similarity to the "
            f"target face ({face.model} cosine similarity {face.cosine_similarity:.3f}, distance {face.distance:.3f})."
        )
    return make_evidence(
        EvidenceType.FACE_SIMILARITY,
        _FACE_STRENGTH[face.match_band],
        text,
        source_url,
        provider=f"local:{face.model}",
        face=face,
    )
